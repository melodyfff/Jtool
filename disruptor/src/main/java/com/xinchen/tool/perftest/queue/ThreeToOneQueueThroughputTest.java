...
import com.xinchen.tool.perftest.support.ValuePublisherQueue;
 ...
 * @author xinchen
 * @version 1.0
 * @date 01/07/2020 13:53
 */
public class ThreeToOneQueueThroughputTest extends AbstractPerfTestQueue {
    ...
    private final ValueQueueProcessorAddition queueProcessor = new ValueQueueProcessorAddition(blockingQueue, ((ITERATIONS / NUM_PUBLISHERS) * NUM_PUBLISHERS) - 1L);
    private final ValuePublisherQueue[] valuePublisherQueues = new ValuePublisherQueue[NUM_PUBLISHERS];

    {
        for (int i = 0; i < NUM_PUBLISHERS; i++) {
            valuePublisherQueues[i] = new ValuePublisherQueue(cyclicBarrier, blockingQueue, ITERATIONS / NUM_PUBLISHERS);
        }
    }
    ...
    protected long runQueuePass() throws Exception {
        ...
        for (int i = 0; i < NUM_PUBLISHERS; i++) {
            futures[i] = executor.submit(valuePublisherQueues[i]);
        }
