...
public final class PingPongSequencedLatencyTest {
    ...
    private final RingBuffer<ValueEvent> pingBuffer = createSingleProducer(ValueEvent.EVENT_FACTORY, BUFFER_SIZE, new BlockingWaitStrategy());
    private final RingBuffer<ValueEvent> pongBuffer = createSingleProducer(ValueEvent.EVENT_FACTORY, BUFFER_SIZE, new BlockingWaitStrategy());
    ...
    private final BatchEventProcessor<ValueEvent> pingProcessor = new BatchEventProcessor<ValueEvent>(pongBuffer, pongBarrier, pinger);

    private final SequenceBarrier pingBarrier = pingBuffer.newBarrier();
    private final Ponger ponger = new Ponger(pongBuffer);
    private final BatchEventProcessor<ValueEvent> pongProcessor = new BatchEventProcessor<ValueEvent>(pingBuffer, pingBarrier, ponger);
